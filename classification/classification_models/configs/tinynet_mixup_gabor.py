...
# Training pipeline with Gabor filter
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='RandomResizedCrop', scale=224, backend='pillow', interpolation='bicubic'),
    dict(type='RandomFlip', prob=0.5, direction='horizontal'),
    
    # GABOR FILTER - Apply with 50% probability as augmentation
    dict(
        type='GaborFilter',
        prob=0.5,  # Apply to 50% of images (for augmentation)
        ksize=21,
        sigma=5.0,
        lambd=10.0,
        gamma=0.5,
        num_orientations=4,
        concatenate=False  # Average filtered images
    ),
...
val_evaluator = [
    ...
    dict(type='SingleLabelMetric', items=['loss'])
]
...
vis_backends = [
    ...
    dict(
        ...
        init_kwargs=dict(
            ...
            name='tinynet_mixup_gabor_mmpretrain',
        )
    )
]
